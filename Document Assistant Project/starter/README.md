...
`src/schemas.py` defines Pydantic schemas for the structured agent outputs. `AnswerResponse` includes the original question, answer text, source document IDs, a UTC timestamp, and a `confidence` field constrained to `0 <= confidence <= 1`. `UserIntent` restricts `intent_type` to `qa`, `summarization`, `calculation`, or `unknown`, and also constrains confidence to the same range. These schemas are passed to `llm.with_structured_output(...)` for intent and memory updates, and to the LangGraph ReAct agent through `response_format=...` for task responses.

### State, Routing, and Memory

`src/agent.py` builds a LangGraph `StateGraph` with this flow:

```text
classify_intent -> qa_agent | summarization_agent | calculation_agent -> update_memory -> END
```

The `classify_intent` node uses the current user input plus recent conversation history to choose the next node. `actions_taken` uses the `operator.add` reducer so every node that runs is recorded. The workflow is compiled with `InMemorySaver`, and `src/assistant.py` passes `thread_id`, `llm`, `tools`, and `retriever` through the workflow config so each session has persistent graph memory.

Sessions are saved under `sessions/` as JSON files. Tool calls are saved under `logs/`; when a session starts, the `ToolLogger` switches to `logs/session_<session_id>.json`, giving each session its own tool-call history.

### Tools

`src/tools.py` implements the calculator as a LangChain `@tool`. It accepts a math expression string, allows only digits, whitespace, parentheses, decimal points, and arithmetic operators, evaluates with disabled builtins, logs each call, and always returns a string result such as `"5"`. Invalid or unsafe input returns a graceful error string and is logged.

### Prompting

`src/prompts.py` includes a structured intent classification prompt with intent categories, examples, confidence scoring guidance, and reasoning instructions. `get_chat_prompt_template(intent_type)` selects the correct system prompt for Q&A, summarization, or calculation. The calculation prompt explicitly requires document retrieval and calculator-tool use for all arithmetic.
...
Assistant: The service agreement is a 12-month contract between DocDacity Solutions Inc. and Healthcare Partners LLC...
...
Assistant: I retrieved the invoices, formed the expression 20000 + 69300 + 214500, and used the calculator tool. The result is 303800.
Intent: calculation
Tools Used: calculator, document_search, document_reader
