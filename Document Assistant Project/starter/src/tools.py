...
class ToolLogger:
    ...
    def set_session(self, session_id: str = None):
        ...
        if session_id:
            ...
            self.log_file = os.path.join(self.logs_dir, f"tool_usage_{timestamp}.json")
...
def create_calculator_tool(logger: ToolLogger):
    """Create a safe calculator LangChain tool."""
    @tool
    def calculator(expression: str) -> str:
        ...
        Validation rules:
        - Only digits, whitespace, decimal points, sqrt(), parentheses and operators are allowed: +, -, *, /, %, **
        - No letters, underscores, or other characters allowed
        - Uses regex validation to reject dangerous expressions

        Evaluation:
        - Uses Python's `eval()` with builtins disabled (`{"__builtins__": None}`)
        - Returns string output only
        - Logs usage via the provided `logger`

        Args:
            expression: Mathematical expression to evaluate (e.g., "(100.5 + 200) / 3")

        Returns:
            Result as string, or an error message string on failure.
        """
        try:
            ...
            # Allow arithmetic syntax plus a small allowlist of math functions.
            allowed_pattern = r"^[0-9\s\+\-\*\/\%\.\(\)sqrt]+$"
            if not re.fullmatch(allowed_pattern, expr):
                msg = (
                    "Error: Expression contains invalid characters. "
                    "Allowed characters: digits, whitespace, sqrt(), + - * / % . and parentheses."
                )
                logger.log_tool_use("calculator", {"expression": expression}, {"error": msg})
                return msg

            if re.search(r"[a-zA-Z_]", expr.replace("sqrt", "")):
                msg = "Error: Only the sqrt() math function is allowed"
                logger.log_tool_use("calculator", {"expression": expression}, {"error": msg})
                return msg

            if re.search(r"[\+\-/\%]{2,}|\*{3,}", expr):
                msg = "Error: Expression has invalid operator sequences"
                logger.log_tool_use("calculator", {"expression": expression}, {"error": msg})
                return msg
            ...
            output = str(result)
            logger.log_tool_use("calculator", {"expression": expression}, {"result": output})
            return output

        except Exception as e:
            error_msg = f"Calculator error: {e}"
...
def create_document_search_tool(retriever, logger: ToolLogger):
    ...
    def document_search(
        ...
        Args:
            ...
            document_id: Exact document ID filter (e.g., 'INV-001')
            category: Metadata category filter (e.g., 'financial', 'medical', 'legal')
        ...
        try:
            ...
            # Keep tool retrieval aligned with graph retrieval: document_id,
            # doc_type, and category filters are applied before scoring.
            if document_id or search_type == "keyword":
                filters = {}
                if document_id:
                ...
                results, search_diagnostics = retriever.retrieve(query, filters=filters, top_k=5)
            ...
            logger.log_tool_use(
                ...
                {"results_count": len(results), "diagnostics": search_diagnostics}
            )
