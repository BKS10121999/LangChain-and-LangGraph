...
class EnergyUsage(Base):
    ...
    consumption_kwh = Column("energy_kwh", Float, nullable=False)
    ...
    cost_usd = Column("cost", Float, nullable=True)  # Cost at time of usage
...
class SolarGeneration(Base):
    ...
    id = Column(Integer, primary_key=True)
    timestamp = Column(DateTime, nullable=False, index=True)
    generation_kwh = Column("solar_kwh_generated", Float, nullable=False)
    weather_condition = Column(String(50), nullable=True)  # e.g., "sunny", "cloudy", "rainy"
    battery_storage_level = Column(Float, nullable=True)
    exported_to_grid_kwh = Column(Float, nullable=True)
